...
The user provides a `dswin_synchronised_window_frame_callback`, called once per frame.
...
typedef void(*dswin_synchronised_window_frame_callback)(
    dswin_synchronised_window*  synchronised_window,        // the window
    dgx_render_target*          render_target,              // the target to render to
    ...
    dgx_gpu_signal*             can_render_signal,          // gate signal to frame rendering
    uint32_t*                   user_wait_signals_count,    // user_wait_signals count (result from function)
    dgx_gpu_signal***           user_wait_signals           // signals to wait on before frame presentation (result from function)
);

typedef struct dswin_synchronised_window_create_info {
    const char*                                     title;
    uint32_t                                        width;
    uint32_t                                        height;
    uint32_t                                        desired_frames_in_flight;
    dswin_synchronised_window_frame_callback        new_frame_callback;
    dgx_window_render_targets_recreated_callback    render_target_recreated_callback;
} dswin_synchronised_window_create_info;

dswin_synchronised_window* dswin_create_synchronised_window(dgx_hardware*, const dswin_synchronised_window_create_info*);
void dswin_free_synchronised_window(dswin_synchronised_window*);
...
void dswin_synchronised_window_enter(dswin_synchronised_window*, int do_lock);
...
struct dswin_synchronised_window {
    dgx_hardware*       owning_hardware;
    dgx_hardware_queue* graphics_queue;
    dgx_window*         window;
    ...
    dgx_cpu_signal**    render_finished_cpu;

    // ensures presentation of frame after rendering finishes
    dgx_gpu_signal**    render_finished_gpu;
    ...
    dgx_gpu_signal**    render_target_ready_for_self;

    // enables user to enqueue rendering as soon as frame is acquired
    dgx_gpu_signal**    render_target_ready_for_user;
};

dswin_synchronised_window* dswin_create_synchronised_window(dgx_hardware* hardware, const dswin_synchronised_window_create_info* info) {
    dswin_synchronised_window* synchronised_window = malloc(sizeof(dswin_synchronised_window));
    *synchronised_window = (dswin_synchronised_window){
    ...
    dgx_window_create_info window_create_info = {
        .title                              = info->title,
        .width                              = info->width,
        .height                             = info->height,
    ...
    synchronised_window->window = dgx_create_window(hardware, &window_create_info);

    dgx_cpu_signal_create_info cpu_create_info = {
        .initialy_signaled = 1
    };

    dgx_hardware_query_queues(hardware, dgx_hardware_queue_type_graphics, 0, 1, &synchronised_window->graphics_queue);

    synchronised_window->render_finished_cpu = malloc(sizeof(dgx_cpu_signal*) * synchronised_window->frames_in_flight);
    synchronised_window->render_finished_gpu = malloc(sizeof(dgx_gpu_signal*) * synchronised_window->frames_in_flight);
    synchronised_window->render_target_ready_for_self = malloc(sizeof(dgx_gpu_signal*) * synchronised_window->frames_in_flight);
    synchronised_window->render_target_ready_for_user = malloc(sizeof(dgx_gpu_signal*) * synchronised_window->frames_in_flight);

    for (uint32_t i = 0; i < synchronised_window->frames_in_flight; i++) {
        synchronised_window->render_finished_cpu[i] = dgx_create_cpu_signal(hardware, &cpu_create_info);
        synchronised_window->render_finished_gpu[i] = dgx_create_gpu_signal(hardware);
        synchronised_window->render_target_ready_for_self[i] = dgx_create_gpu_signal(hardware);
        synchronised_window->render_target_ready_for_user[i] = dgx_create_gpu_signal(hardware);
    }

    return synchronised_window;
}

void dswin_free_synchronised_window(dswin_synchronised_window* synchronised_window) {
    for (uint32_t i = 0; i < synchronised_window->frames_in_flight; i++) {
        dgx_free_cpu_signal(synchronised_window->render_finished_cpu[i]);
        dgx_free_gpu_signal(synchronised_window->render_finished_gpu[i]);
        dgx_free_gpu_signal(synchronised_window->render_target_ready_for_self[i]);
        dgx_free_gpu_signal(synchronised_window->render_target_ready_for_user[i]);
    }
    ...
    dgx_free_window(synchronised_window->window);
    free(synchronised_window);
}

void dswin_synchronised_window_enter(dswin_synchronised_window* synchronised_window, int do_lock) {
    uint32_t current_frame = synchronised_window->current_frame;

    // Wait for previous cycle to finish
    if (do_lock) {
        dgx_cpu_signal_wait (synchronised_window->render_finished_cpu[current_frame]);
        dgx_cpu_signal_reset(synchronised_window->render_finished_cpu[current_frame]);
    }
    else {
        if (!dgx_cpu_signal_signaled(synchronised_window->render_finished_cpu[current_frame])) return;
        dgx_cpu_signal_reset(synchronised_window->render_finished_cpu[current_frame]);
    }

    // Accquire next frame render target
    uint32_t drawn_target_index = dgx_window_acquire_next_render_target_index(
    ...
    dgx_gpu_signal* render_target_ready_signals[] = {
    ...
    dgx_submit_info multiply_submit_info = {
    ...
    dgx_submit_command_list(synchronised_window->graphics_queue, &multiply_submit_info);
    ...
    dgx_gpu_signal**    render_finished_signals         = NULL;
    synchronised_window->new_frame_callback(
        synchronised_window,
        dgx_window_get_render_target(synchronised_window->window, drawn_target_index),
        current_frame,
        synchronised_window->render_target_ready_for_user[current_frame],
    ...
    dgx_gpu_signal* waits[render_finished_signals_count + 1];
    ...
    dgx_submit_info render_wait_submit_info = {
        .command_lists_count        = 0,
        .command_lists              = NULL,
    ...
    dgx_submit_command_list(synchronised_window->graphics_queue, &render_wait_submit_info);

    // Present, and move onto next frame
    dgx_window_enqueue_render_target_present(
...
dgx_window* dswin_synchronised_window_get_window(dswin_synchronised_window* synchronised_window) {
